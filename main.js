/*csslint vars: true, plusplus: true, devel: true, nomen: true, regexp: true, indent: 4, maxerr: 50 */
/*global define, $, brackets, window, log , Mustache, NodeConnection */

define(function (require, exports, module) {
    "use strict";
    
	var CONFIG_FILE = 'css.concat';
	var CONTEXT_MENU_TEXT = 'Concatenate CSS files';
	...
	var parseConfigFile = function (data, callback) {
		...
		outputFile = 'css-concat-build.css';
		concatOnSave = false;
		fileList = [];
	...
	var loadConfigFile = function (callback) {
		...
		file.read(function (error, data, status) {
			if (error) {
				console.log('[brackets-css-concat] Error loading project config file (' + CONFIG_FILE + '): ' + error);
			} else {
	...
	var readFile = function (fileName, readInto, callback) {
		...
		file.read(function (error, data) {
			if (error) {
				console.error('[brackets-css-concat] Error reading file for concatenation (' + fileName + '): ' + error);
			} else {
				readInto += data + '\n';
			}
	...
	var writeBuildFile = function () {
		getFileListContents(function (data) {
			...
			buildFile.write(data, function (error, stats) {
				if (error) {
					console.error('[brackets-css-concat] Error writing build file: ' + error);
				} else {
					//ProjectManager.refreshFileTree();
					console.log('[brackets-css-concat] ' + new Date().toLocaleTimeString() + ' Concatenation done succesfully! Duration: ' + (new Date().getTime() - time) + ' ms.');
				}
			});
		});
	};
			...
			if (!error) {
				...
				directory.create(function (error, status) {
					if (error && error !== 'AlreadyExists') {
						console.error('[brackets-css-concat] Error creating build directory: ' + error);
						return;
					}
